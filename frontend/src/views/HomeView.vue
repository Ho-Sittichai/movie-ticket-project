...
const fetchMovies = async () => {
  try {
    ...
    error.value = 'Failed to load movies. Please check if the server is running.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="container mx-auto px-6 py-10 min-h-screen">
    <div class="flex items-center justify-between mb-8">
      ...
      <span class="text-gray-500 text-sm">Showing {{ movies.length }} Movies</span>
    </div>
    
    <div v-if="loading" class="flex flex-col items-center justify-center py-20 bg-black/20 rounded-3xl border border-white/5 backdrop-blur-sm">
      <div class="w-12 h-12 border-4 border-brand-red border-t-transparent rounded-full animate-spin mb-4 shadow-[0_0_15px_rgba(229,9,20,0.5)]"></div>
      <p class="text-gray-400 font-medium animate-pulse">Loading amazing movies...</p>
    </div>

    <div v-else-if="error" class="bg-red-500/10 border border-red-500/20 p-6 rounded-2xl text-center max-w-lg mx-auto backdrop-blur-md">
      <p class="text-red-400 font-medium mb-4">{{ error }}</p>
      <button @click="fetchMovies" class="px-6 py-2 bg-brand-red text-white rounded-xl hover:bg-red-700 transition-all font-semibold shadow-lg shadow-red-900/20">
        Try Again
      </button>
    </div>

    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
      <div v-for="movie in movies" :key="movie.id" class="group relative bg-[#1A1A1A] rounded-2xl overflow-hidden hover:scale-[1.03] transition-all duration-300 shadow-xl hover:shadow-2xl hover:shadow-brand-red/10 border border-white/5 flex flex-col h-full">
